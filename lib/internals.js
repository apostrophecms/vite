...
module.exports = (self) => {
  return {
    ...
    async shouldBuild(id, options) {
      ...
      // Here we are only interested if we should force a "rebuild" or we can use the cache.
    ...
    setWatchVoters(entrypoints) {
      ...
      for (const entrypoint of entrypoints) {
        self.entrypointWatchVoters[entrypoint.name] = (relSourcePath, rootPath) => {
          if (self.apos.asset.getEntrypointManger(entrypoint).match(relSourcePath, rootPath)) {
            return entrypoint;
          }
          return null;
        };
    ...
    onSourceChange(filePath, silent = false) {
      ...
      // - this can be solved with a separate core handler that does only that (it's tricky).
      // if (silent) {
      //   return;
      // }
      ...
      //   if (hasBundledChange) {
      //     console.log('onSourceChange bundled restartId');
      //     self.apos.asset.forcePageReload();
      //     return;
      //   }
      // }
    ...
    onSourceUnlink(filePath, isDir) {
      ...
      const entrypoints = self.getChangedEntrypointsFor(source, self.currentSourceMeta[index]);
    ...
    // In scenarios where the module build is not tirggered at all (e.g. boot in production),
    ...
    // tje `devServer` property) when a build for given entrypoint is not triggered (because
    // this entrypoint is served by the dev server).
    ...
    // to `devServer` and `types` (can be string 'public' or 'apos'). This will allow us to
    // introduce new entrypoint types and features without breaking the current logic.
     ...
     * Return a function that accepts Vite Environment object and returns the merged Vite config.
     *
     * @param {import('vite').UserConfig} baseConfig
     * @returns {Promise<(configEnv: import('vite').ConfigEnv) => Promise<import('vite').UserConfig>>}
     */
     ...
     * Return a function that accepts Vite Environment object and returns the merged Vite config.
     * The project level configuration provided by modules and a root level `apos.vite.config.js`
     * will be merged with the base and public configurations.
     *
     * @param {import('vite').UserConfig} baseConfig
     * @return {Promise<(configEnv: import('vite').ConfigEnv) => Promise<import('vite').UserConfig>>}
     */
    async getPublicViteConfig(baseConfig) {
      ...
      const mergeConfigs = vite.defineConfig(async (configEnv) => {
        ...
        // The `apos.vite.config.js` at the project root can be used to extend the public config.
