...
module.exports = (self) => {
  return {
    async initWhenReady() {
      ...
      await self.computeResolvePackages(self.userConfigFile);

      await fs.mkdir(self.buildRootSource, { recursive: true });
    },

    // Computes an ordered, de-duplicated list of package names that the
    // pnpm resolver plugin should consider as resolution roots.
    //
    // Configuration sources (in priority order):
    // 1. Module options (self.options.resolvePackageRoots) - highest priority
    // 2. Vite config file (apos.vite.config.js/mjs - aposVite.resolvePackageRoots)
    // 3. package.json (aposVite.resolvePackageRoots) - lowest priority
    //
    // Configuration values:
    // - true: enable with default packages ['@apostrophecms/vite', 'apostrophe']
    // - false or []: disable the plugin
    // - ['pkg1', 'pkg2']: custom packages (merged with defaults)
    // - undefined/null: no configuration found (plugin uses its own defaults)
    //
    // Examples:
    //
    // Module options:
    ...
    //   '@apostrophecms/vite': {
    //     options: {
    //       resolvePackageRoots: true // or ['some-package']
    //     }
    //   }
    // }
    //
    // package.json:
    // "aposVite": {
    //   "resolvePackageRoots": ["some-package", "another-package"]
    // }
    //
    ...
    //   }
    // }
    async computeResolvePackages(viteConfigFile) {
      const defaults = [ '@apostrophecms/vite', 'apostrophe' ];

      // Helper to get module options config (highest priority)
      const getModuleOptionsConfig = () => {
        if (!self.options?.resolvePackageRoots) {
          return null;
        }
        const config = self.options.resolvePackageRoots;
        return config === undefined ? null : config;
      };

      // Helper to load vite config (medium priority)
      const loadViteConfig = async (configFile) => {
        if (!configFile) {
          return null;
        }
        try {
          ...
          if (!userConfig?.aposVite) {
            return null;
          }
          const config = userConfig.aposVite.resolvePackageRoots;
          return config === undefined ? null : config;
        } catch {
          return null;
        }
      ...
      // Helper to load package.json config (lowest priority)
      const loadPackageJsonConfig = async () => {
        try {
          const pkgPath = path.join(self.apos.rootDir, 'package.json');
          const pkg = await fs.readJson(pkgPath);
          if (!pkg?.aposVite) {
            return null;
          }
          const config = pkg.aposVite.resolvePackageRoots;
          return config === undefined ? null : config;
        } catch {
          return null;
        }
      ...
      // Load configs with priority: module options > vite config > package.json
      let activeConfig = getModuleOptionsConfig();
      if (activeConfig === null) {
        activeConfig = await loadViteConfig(viteConfigFile);
      }
      if (activeConfig === null) {
        activeConfig = await loadPackageJsonConfig();
      }

      if (activeConfig === null) {
        // No configuration found - leave undefined (plugin will use its own defaults)
        return;
      }

      if (activeConfig === true) {
        // Explicitly enabled with defaults
        self.resolvePackages = defaults;
        return;
      }

      if (
        activeConfig === false ||
        (Array.isArray(activeConfig) && activeConfig.length === 0)
      ) {
        // Explicitly disabled
      ...
      if (Array.isArray(activeConfig)) {
        // Custom package list - merge with defaults
        self.resolvePackages = [ ...defaults, ...activeConfig ];
        return;
      }

      // Invalid config - disable plugin
      self.resolvePackages = [];
    },
    ...
    async getBaseViteConfig(id, options, env) {
      return viteBaseConfig({
        ...
        root: self.buildRoot,
        resolveRoot: self.apos.npmRootDir,
        resolvePackages: self.resolvePackages,
