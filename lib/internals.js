...
module.exports = (self) => {
  return {
    ...
    // Builds an ordered, de-duplicated list of package names that the
    // pnpm resolver plugin should consider as additional resolution roots.
    // Sources (project level, in order): package.json, apos.vite.config.*
    // pacakge.json example:
    // "aposVite": {
    //   "resolvePackageRoots": [ "my-package", "another-package" ]
    // }
    // apos.vite.config.* example:
    // aposVite: {
    //   resolvePackageRoots: [ 'my-package', 'another-package' ]
    // }
    // Empty array means no resolution (plugin disabled).
    // Otherwise defaults [ '@apostrophecms/vite', 'apostrophe' ] are
    // always included.
    async computeResolvePackages(viteConfigFile) {
      const core = [ '@apostrophecms/vite', 'apostrophe' ];
      let hasPakcageJsonCfg = false;
      const packageJsonPkgs = new Set();
      let hasViteCfg = false;
      const viteConfigPkgs = new Set();

      try {
        const pkgPath = path.join(self.apos.rootDir, 'package.json');
        const pkg = await fs.readJson(pkgPath);
        hasPakcageJsonCfg = !!pkg?.aposVite;
        const extra = pkg?.aposVite?.resolvePackageRoots;
        if (Array.isArray(extra)) {
          for (const name of extra) {
            if (typeof name === 'string' && name.trim()) {
              packageJsonPkgs.add(name.trim());
            }
          }
        }
      } catch (e) {
      ...
      try {
        if (viteConfigFile) {
          ...
          hasViteCfg = !!userConfig?.aposVite;
          const extra = userConfig?.aposVite?.resolvePackageRoots;
          if (Array.isArray(extra)) {
            for (const name of extra) {
              if (typeof name === 'string' && name.trim()) {
                viteConfigPkgs.add(name.trim());
              }
            }
          }
        }
      } catch (e) {
        // Ignore failures to load the user config file for this purpose
      }

      let finalSet;
      if (hasViteCfg) {
        finalSet = viteConfigPkgs;
      } else if (hasPakcageJsonCfg) {
        finalSet = packageJsonPkgs;
      }

      if (finalSet) {
        self.resolvePackages = finalSet.size
          ? new Set([ ...core, ...Array.from(finalSet) ])
          : [];
        return;
      }

      // This results in `undefined` passed to the config, and
      // the plugin defaults apply.
      if (!hasViteCfg && !hasPakcageJsonCfg) {
        return;
      }

      // Disable the plugin in all other cases.
